...
class ModelTrainer:
    ...
    def get_gpu_memory(self, device=0):
        """获取GPU内存使用情况"""
        if torch.cuda.is_available():
            return torch.cuda.memory_allocated(device), torch.cuda.memory_reserved(device)
        return 0, 0

    def is_gpu_memory_available(self, threshold=0.9, device=0):
        """检查指定GPU内存是否有足够空间"""
        if torch.cuda.is_available():
    ...
    def get_available_gpu(self):
        ...
        for i in range(torch.cuda.device_count()):
            if self.is_gpu_memory_available(threshold=0.9, device=i):
                available_gpus.append(i)
        return available_gpus

    def get_gpu_utilization(self, device=0):
        """获取GPU内存使用率"""
        if torch.cuda.is_available():
            allocated = torch.cuda.memory_allocated(device)
            total = torch.cuda.get_device_properties(device).total_memory
    ...
    def train_model(self, model, train_data, optimizer, epochs, device, max_time=24*3600):
        """添加时间限制的训练函数"""
        ...
        try:
            for e in range(epochs):
                ...
                for d in train_data:
                    # 监控GPU使用
                    current_gpu_util = self.get_gpu_utilization(device)
    ...
    def plot_test_results(self, predictions, true_labels, m):
        """绘制测试结果图"""
        plt.figure(figsize=(10, 5))
    ...
    def batch_process_folders(self, folder_list, epochs, wait_time=30):
        """优化的批处理函数"""
        num_gpus = torch.cuda.device_count()
        results = {}

        # 按GPU分配文件夹
        gpu_folders = {gpu_id: [] for gpu_id in range(num_gpus)}

        # 处理每个GPU的任务
        for gpu_id in range(num_gpus):
            device = f"cuda:{gpu_id}"
            logger.info(f"GPU {gpu_id} 开始处理任务")
            ...
            while folder_list:
                ...
                # 每次放入10个文件夹
                batch_folders = folder_list[:10]
                folder_list = folder_list[10:]

                # 使用多线程处理每个文件夹
                threads = []
                for folder in batch_folders:
                    thread = threading.Thread(target=self.process_folder, args=(folder, gpu_id, epochs, results))
                    thread.start()
    ...
    def process_folder(self, folder, gpu_id, epochs, results):
        """处理单个文件夹的训练任务"""
        device = f"cuda:{gpu_id}"
        try:
            # 设置每个任务的超时时间
            ...
            losses, accuracies = self.train_model(model, train_loader, optimizer, epochs, device)
            ...
            self.make_dir(os.path.join('train', folder))
            self.plot(losses, accuracies, os.path.join('train', folder))
            ...
            test_loss, test_accuracy, predictions, true_labels = self.evaluate_model(model, test_loader, device)
            ...
            # 绘制测试结果图
            self.plot_test_results(predictions, true_labels, os.path.join('train', folder))
...
if __name__ == "__main__":
    ...
    try:
        ...
        results = trainer.batch_process_folders(
            folder_list,
            args.m,
            wait_time=args.wait
        )
